...
def train(model, train_loader, val_loader, loss_fn, optimizer, scheduler, epochs = 5,  model_name = 'resnet_re_train', save_path = 'models', fine_tune = False):
    ...
    for epoch in range(epochs):
        ...
        with open(f'cnn_train_losses_no_aggregate_{fine_tune}.txt', 'w') as f:
            for line in all_train_losses:
                f.write(f"{line}\n")
        with open('cnn_train_losses_epoch.txt', 'w') as f:
            for line in train_losses:
                f.write(f"{line}\n")
        
        with open(f'cnn_eval_losses_no_aggregate_{fine_tune}.txt', 'w') as f:
            for line in all_val_losses:
                f.write(f"{line}\n")
        with open(f'cnn_eval_losses_epoch_{fine_tune}.txt', 'w') as f:
            for line in val_losses:
                f.write(f"{line}\n")
                
        with open(f'cnn_train_avg_prec_epoch_{fine_tune}.txt', 'w') as f:
            for line in train_avg_prec_list:
                f.write(f"{line}\n")
                
        with open(f'cnn_eval_avg_prec_epoch_{fine_tune}.txt', 'w') as f:
...
if __name__ == "__main__":
    ...
    train_losses, val_losses, train_avg_prec_list, val_avg_prec_list = train(resnet, train_loader, val_loader, loss_fn, optimizer, scheduler, epochs = 5, model_name = params['model_name'], fine_tune = False)
    
    for n, p in resnet.named_parameters():
        p.requires_grad = True
        
    optimizer = torch.optim.Adam(resnet.parameters(), lr = 0.0001, weight_decay = 0.01)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = len(train_loader) * 5)

    
    train_losses, val_losses, train_avg_prec_list, val_avg_prec_list = train(resnet, train_loader, val_loader, loss_fn, optimizer, scheduler,  epochs = 5, model_name = 'resnet_re_train_fine_tuned', fine_tune = True)